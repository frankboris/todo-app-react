...
export const appliedStatusFilter = (filter) => ({
    type: types.SET_STATUS_FILTER,
    filter: filter
});

export const appliedOrderByFilter = (filter) => ({
    type: types.SET_ORDER_BY_FILTER,
    filter: filter
});

export const appliedContentFilter = (filter) => ({
...
export const fetchTodos = (filter, page) => {
    return (dispatch) => {
        ...
        axios
            .get(`http://localhost/BlogReact/public/api/v1/todos?page=${page}&filter=${JSON.stringify(filter)}`)
            .then(response => {
